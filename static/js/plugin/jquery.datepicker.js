...
(function($) {
    ...
    $.fn.datepicker = function(o) {
        ...
        if (cp.length == 0) {
            ...
            //生成tBody结束
            cpHA.push("</td></tr>");
            cpHA.push("<tr><td class='bbit-dp-bottom' align='center'>", def.showtime ? "<input type='text' value='00:00' maxlength='8' id='BBIT-DP-TIME' class='bbit-dp-time'/>" : "", "<button id='BBIT-DP-TODAY'>", def.btnToday, "</button></td></tr>");
        ...
        function tbhandler(e) {
            ...
            if (!$(td).hasClass("bbit-dp-disabled")) {
                ...
                if (def.showtime) {
                    ...
                    if (arrtime.length == 2) {
                        d.setHours(arrtime[0], arrtime[1]);
                    }
                }
                cp.data("indata", d);
                returndate();
        ...
        function writecb() {
            ...
            var ade = cp.data("ade");
            var bhm = [];
            var tds = def.today.Format(i18n.datepicker.dateformat.fulldayvalue);
            var indata = cp.data("indata");
            var ins = indata != null ? indata.Format(i18n.datepicker.dateformat.fulldayvalue) : "";
            for (var i = 1; i <= 42; i++) {
                if (i % 7 == 1) {
                    bhm.push("<tr>");
                }
                ...
                var s = ndate.Format(i18n.datepicker.dateformat.fulldayvalue);
                if (s == tds) {
                ...
                bhm.push("<td class='", tdc.join(" "), "' title='", ndate.Format(i18n.datepicker.dateformat.fulldayvalue), "' xdate='", ndate.Format(i18n.datepicker.dateformat.fulldayvalue), "'><a href='javascript:void(0);'><em><span>", ndate.getDate(), "</span></em></a></td>");
                if (i % 7 == 0) {
                    bhm.push("</tr>");
                }
            }
        ...
        return $(this).each(function() {
            ...
            picker.click(function(e) {
                ...
                if (v == null || v == "") {
                    ...
                    if (def.showtime) {
                        def.Hour = v.getHours();
                        def.Minute = v.getMinutes();                         
                        def.inputDate = v ;
                        $("#BBIT-DP-TIME").val(def.inputDate.Format("HH:mm"));
                    }
                    else {
                        def.inputDate = v;
                    }
